# ============================================================
# train_gbt.py  (GBT with CV logs, learning curve, FI, PLOT DATA)
# ============================================================
import sys, json
...
MODEL_NAME = "GBT_Tuned"

if len(sys.argv) != 4:
    print("Usage: spark-submit train_gbt.py <train.parquet> <test.parquet> <output_dir>")
    sys.exit(1)
...
cv = CrossValidator(estimator=gbt, estimatorParamMaps=gbt_paramGrid,
                    evaluator=evaluator, numFolds=3, parallelism=28, seed=42)
...
best = {"maxDepth": pm.get(gbt.maxDepth), "maxIter": pm.get(gbt.maxIter)}
...
write_cv_table(spark, cv_model, param_maps, cv_out, ["maxDepth", "maxIter"])
write_learning_curve(spark, base_stages, lambda **kw: GBTRegressor(featuresCol='features', labelCol='label', seed=42, **kw),
